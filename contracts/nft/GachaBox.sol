...
contract GachaBox is Initializable, OwnableUpgradeable {
  ...
  function mintCharacters() private returns (uint256[4] memory characterNFTIds) {
  ...
  function getSpecialCharacterNFTId() private returns (uint256) {
    ...
    uint256 characterNFTId = ILordArenaCharacter(lordArenaCharacter).safeMint(msg.sender, characterId, qualityId);
    ...
    )
  {
    ...
    uint256[4] memory characterNFTIds = mintCharacters();
