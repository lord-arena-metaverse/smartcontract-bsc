...
import { deployInitializableAdminUpgradeabilityProxy, deployLordArenaTokenDev} from '../../helpers/contracts-deployments';
...
task(`deploy-${LordArenaTokenDev}`, `Deploys the ${LordArenaTokenDev} contract`)
  ...
  .setAction(async ({ verify, vaultAddress, aaveAddress }, localBRE) => {
    ...
    const lastTokenDevImpl = await deployLordArenaTokenDev(verify);
